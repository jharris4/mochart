...
export default function EditableChart(props: Props) {
  ...
  function buildFilteredState(prev: EditableState, partial: Partial<EditableState>, filteredData: Row[], removedData: Row[], resetGroupIndex: boolean): EditableState {
    ...
    if (resetGroupIndex === true) {
      next.groupIndex = -1;
      next.seriesValuesText = "Select a Group";
    }
  ...
  if (selectionMode === 'group') {
    controlContent = (
      <div className="chart-controls-container">
        ...
        <span className="chart-controls-input">
          <Form inline>
            <Input type="text" disabled={error || sequencePlaying} value={groupValuesText} onChange={groupValuesChanged} />
          </Form>
        </span>
      </div>
    );
    ...
    const groupIndexText = 'Group: ';
    const seriesIndexText = 'Series: ';
    ...
    controlContent = (
      <div className="chart-controls-container">
        ...
        <span className="chart-controls-input">
          <Form inline>
            <Input type="text" disabled={error || seriesControlsDisabled} value={seriesValuesText} onChange={seriesValuesChanged} />
          </Form>
        </span>
      </div>
    );
