...
export default function getDescriptions() {
  return {
    ...
    size: 'the depth (in pixels) of the clip indicator band (use "auto" to size it from the label plus padding on both sides)',
    padding: 'the space (in pixels) between the clip indicator label and the edges of its band, which also determines the band depth when size is "auto"',
    ...
    hatch: {
      ...
      properties: {
        ...
        width: 'the thickness (in pixels) of each hatch line; at or above spacing the hatch closes up into a flat fill'
      }
    },
    showInFront: 'whether the clip indicator should be shown in front (true) or behind (false) the series shapes'
  };
}
