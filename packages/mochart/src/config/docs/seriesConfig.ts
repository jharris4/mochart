...
export default function getDescriptions() {
  return {
    ...
    labelAboveBaseMinPositionFraction: 'the minimum position fraction (0 - 1) above the base value for which series labels should be shown (use null for none)',
    labelAboveBaseMaxPositionFraction: 'the maximum position fraction (0 - 1) from the domain maximum for which series labels should be shown (use null for none)',
    labelBelowBaseMinPositionFraction: 'the minimum position fraction (0 - 1) from the domain minimum for which series labels should be shown (use null for none)',
    labelBelowBaseMaxPositionFraction: 'the maximum position fraction (0 - 1) below the base value for which series labels should be shown (use null for none)',
...
export function getDetails() {
  return {
    ...
    skipMissing: 'When `true`, categories whose value is missing (`undefined`) are left out of the shape, so lines and areas connect directly between the neighbouring defined values; when `false` the shape breaks at the gap. For a series with a `rangeProperty`, a category counts as missing only when both properties are undefined — see `skipPartialRange`.',
    skipPartialRange: 'Only affects series with a `rangeProperty` (stacked series are unaffected). By default a category with just one of `property`/`rangeProperty` undefined keeps a zero-extent span collapsed at the defined value, so ranged areas stay connected through it. When `true` such categories count as missing instead, following the configured missing-value treatment: a break in the shape, or skipped over when `skipMissing` is set, or drawn at the base when `showMissingAtBase` is set.',
