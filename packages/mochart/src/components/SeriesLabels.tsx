...
export default class SeriesLabels extends PureComponent {
  constructor(props) {
    super(props);
  }
      ...
      if (domainMin !== null) {
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (getDefined(null, i) && labelValues[skipI] !== void 0 && withinPercentages(maxValues[skipI], minValues ? minValues[skipI] : null)) {
            ...
            labels.push(
              <text key={'label-'+i} className={mochartCssClasses['seriesLabel']+i} transform={translate(x, y)}
                    textAnchor={textAnchor} dy={dy} stroke={labelStrokeColor} fill={labelFillColor} fillOpacity={labelFillOpacity} strokeOpacity={labelStrokeOpacity}
                    strokeWidth={labelStrokeWidth} onMouseEnter={() => onGroupEnter(i)} onMouseLeave={() => onGroupLeave(i)} onClick={() => onGroupClick(i)}>
                {valueFormat(labelValues[skipI])}
              </text>
            );
          }
        }
        return (
          <g className={mochartCssClasses['seriesLabels']}>
            {labels}
          </g>
        );
      }
