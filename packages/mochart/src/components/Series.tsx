...
import { NONE, RENDERER_AREA, RENDERER_LINE, RENDERER_BAR, RENDERER_NONE, COLOR_GROUP_INDEX } from '../config/core/constants';
...
export default class Series extends PureComponent {
  ...
  render() {
    ...
    if (filteredValues.plain !== null) {
      ...
      if (seriesConfig.renderer === RENDERER_LINE) { // TODO - consider drawing a second line for range series...
        ...
        seriesShape = (
          <path d={lineGenerator()} className={mochartCssClasses['seriesLine']} strokeWidth={seriesStrokeWidth}
        ...
        seriesShape = (
          <path d={areaGenerator()} className={mochartCssClasses['seriesArea']} strokeWidth={seriesStrokeWidth}
        ...
        for (let i=0; i<seriesPositionData.length; i++) {
          if (seriesPositionData.getDefined(null, i)) {
            ...
            bars.push(
              <path key={'bar-'+i} d={columnGenerator(i)} className={mochartCssClasses['seriesBar']+i}
      ...
      else if (seriesConfig.renderer === RENDERER_NONE) {
        seriesShape = false;
      }
      return (
        <g className={mochartCssClasses['series'] + seriesId}
           transform={translateObject(seriesLayoutInfo)}>
          {seriesShape}
          <SeriesMarkers colorPaletteConfig={colorPaletteConfig} seriesConfig={seriesConfig} seriesPositionData={seriesPositionData}
                         filteredValues={filteredValues} rawDomains={rawDomains} inverted={inverted} seriesIndex={seriesIndex}
                         focusData={focusData} onGroupEnter={onGroupEnter} onGroupLeave={onGroupLeave} onGroupClick={onGroupClick}/>
          <SeriesLabels colorPaletteConfig={colorPaletteConfig} seriesConfig={seriesConfig} seriesAxisScale={seriesAxisScale}
                        rawSeriesAxisDomain={rawSeriesAxisDomain} seriesPositionData={seriesPositionData} filteredValues={filteredValues} inverted={inverted}
                        focusData={focusData} onGroupEnter={onGroupEnter} onGroupLeave={onGroupLeave} onGroupClick={onGroupClick} seriesIndex={seriesIndex}/>
        </g>
      );
    }
    return false;
