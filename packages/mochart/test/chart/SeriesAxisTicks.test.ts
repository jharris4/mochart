...
function renderChart(seriesAxisConfig: Record<string, unknown>) {
  const { createChart, enhanceConfig, ArrayOfObjectsDataProvider } = mochart;
  const mochartConfig = enhanceConfig({
    version: '1.0.0',
    groupAxisConfig: { property: 'label', type: 'string', scale: 'ordinal', visible: false },
    seriesAxisConfigs: [{ id: 'sa', min: 0, max: 3, ...seriesAxisConfig }],
    seriesConfigs: [{ axis: 'sa', property: 'value', renderer: 'bar' }]
  });
