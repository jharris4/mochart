...
/** The interface a member's type names, if it is a plain reference (optionally an array or nullable). */
function memberInterface(type: string, interfaces: Map<string, ParsedInterface>): ParsedInterface | undefined {
...
export function buildEnumerations(configModel: ConfigReferenceModel): EnumerationsResult {
  ...
  const visit = (parsed: ParsedInterface, sectionId: string, pathKeys: string[], stack: string[]) => {
    for (const member of allMembers(parsed, interfaces)) {
      const keys = [...pathKeys, member.key];
      const nested = memberInterface(member.type, interfaces);
      if (nested !== undefined) {
        if (!stack.includes(nested.name)) visit(nested, sectionId, keys, [...stack, nested.name]);
        continue;
      }
      for (const name of mentionedUnions(member.type)) {
