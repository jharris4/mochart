...
applied to every area or bar series automatically when no gradients are configured.
...
- A series can use a pattern only when its `renderer` is `"area"` or `"bar"`.
  For a pie slice, set its otherwise-ignored renderer to one of those values.
