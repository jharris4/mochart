...
applied automatically to every pie slice or area/bar series when no gradients
are configured.
...
- In an XY chart, a series can use a pattern only when its `renderer` is
  `"area"` or `"bar"`. Pie slices can use patterns regardless of the otherwise
  irrelevant series renderer.
